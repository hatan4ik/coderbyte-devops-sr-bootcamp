...
test:
	@echo "Running tests..."
	poetry run bash modules/A_zero_to_hero/run_tests.sh || true
	cd modules/C_full_project && poetry run pytest tests/ -v || echo "pytest not available"

lint:
	@echo "Linting Python files..."
	poetry run ruff check . || true
	poetry run python -m py_compile `find . -name '*.py'` || true
